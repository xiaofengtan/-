...
namespace DataSource
{
    ...
    /// Oracl数据源
    /// </summary>
    public class OraclSource : IDataSourceType, IDisposable
    {
        #region 字段
        /// <summary>
        /// 是否已开始数据库事务处理
        ...
        /// </summary>
        public OracleTransaction SqlTransaction
        {
        ...
        public OraclSource(string connectionstring)
        {
        ...
        public DataSet ExecuteDataSet(CommandType commandtype, string commandtext, params IDataParameter[] parameter)
        {
            if (_connection.State == ConnectionState.Closed)
                _connection.Open();
            OracleCommand commmand = InitSqlCommand(commandtype, commandtext, parameter);
            OracleDataAdapter adapter = new OracleDataAdapter(commmand);
        ...
        public int ExecuteNonQuery(CommandType commandtype, string commandtext, params IDataParameter[] parameter)
        {
            if (_connection.State == ConnectionState.Closed)
                _connection.Open();
            OracleCommand command = InitSqlCommand(commandtype, commandtext, parameter);
        ...
        public int ExecuteNonQuery(IDbConnection conn, CommandType cmdType, string cmdText, params IDataParameter[] parameter)
        {
            ...
            command.Connection = (conn as OracleConnection);
            return command.ExecuteNonQuery();
        ...
        {
            OracleCommand command = InitSqlCommand(cmdType, cmdText, parameter);
            command.Transaction = (trans as OracleTransaction);
            return command.ExecuteNonQuery();
        }
        /// <summary>
        ...
        private OracleCommand InitSqlCommand(CommandType commandtype, string commandtext, params IDataParameter[] parameter)
        {
