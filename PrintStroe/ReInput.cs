...
{
    public partial class ReInput : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            AllOutPaper = Model.Paper_In.GetDataTable(whe);
            if (AllOutPaper != null && AllOutPaper.Rows.Count > 0)
            {
                List<string> titile = Model.Paper_In.GetColumNikeName();
            ...
            {
                MessageBox.Show(dt1str + "没有入库纸张，请重新选择或退出！", "提示");
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    ...
                    {
                        string tt = dataGridView1.Rows[i].Cells[11].Value.ToString();
                        if (tt == "False")
                            IsSelect = true;
                    }
                    if (IsSelect)
        ...
        {
            DataTable upddata = AllOutPaper.Clone();
            DialogResult dr = MessageBox.Show("确认将以下纸张全部撤销入库？", "提示", MessageBoxButtons.YesNo);
            if (dr == DialogResult.Yes && dataGridView1.Rows.Count > 0)
            {
                ...
                //添加出库记录
                int result = Model.Paper_In.RemoveOut(RemoveList);
                ...
                if (result == count)
                {
                    MessageBox.Show("您选择的" + count.ToString() + "已经全部撤销了入库！", "提示");
                }
            }
        }
    }
